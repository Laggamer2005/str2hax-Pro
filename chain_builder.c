...
int main(int argc, char *argv[]) {
	...
	if (!chain) {
		log_error_file("[chain_builder] Error: could not open output file chain.bin");
		exit(1);
	}
	...
	for (size_t i = 0; i < chain_iterations; i++) {
		if (fwrite(chain_layout, sizeof(chain_layout), 1, chain) != 1) {
			log_error_file("[chain_builder] Error: could not write to output file");
			fclose(chain);
			exit(5);
		}
	}
	...
	for (int i = 0; i < 7; i++) {
		if (fwrite(&null_bytes, 4, 1, chain) != 1) {
			log_error_file("[chain_builder] Error: could not write to output file");
			fclose(chain);
			exit(5);
		}
	...
	if (fclose(chain) != 0) {
		log_error_file("[chain_builder] Error: could not close output file");
		exit(5);
	}
